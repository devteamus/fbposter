...
def upload_csv():
    ...
    try:
        ...
        # Validate every media_url before importing anything, so a bad CSV
        # never creates a half-imported job. Only http/https URLs are
        # accepted — this is what gets sent server-side to the Facebook
        # Graph API, so anything else (file://, javascript:, internal IPs
        # via unusual schemes, etc.) must never reach that call.
        bad_rows = []
        ...
        if bad_rows:
            ...
            return jsonify({
                "error": f"Invalid media_url or oversized caption/comment on row(s): {preview}"
                         f"{' ...' if len(bad_rows) >= 10 else ''}. "
                         f"media_url must be a full http(s) link, comment max 2000 chars."
            }), 400
        ...
        for _, row in df.iterrows():
            ptype = str(row.get("post_type", "image")).lower().strip()
